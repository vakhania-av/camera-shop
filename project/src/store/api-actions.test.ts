...
describe('Async actions:', () => {
  ...
  it('should dispatch LOAD_PROMO when GET /promo', async () => {
    ...
    mockApi.onGet(APIRoute.Promo).reply(200, mockPromo);
  ...
  it('should dispatch LOAD_CURRENT_CAMERA when GET /cameras/id', async () => {
    ...
    const mockId = '2';
    mockApi.onGet(`${APIRoute.Cameras}/${mockId}`).reply(200, mockCamera);
  ...
  it('should dispatch LOAD_CAMERAS_PER_PAGE when GET /cameras?_start=start&_limit=limit', async () => {
    ...
    await store.dispatch(fetchCamerasPerPageAction([mockStart, mockLimit]));
  ...
  it('should dispatch POST_REVIEW when POST /reviews', async () => {
    ...
    mockApi.onPost(APIRoute.Reviews).reply(200, mockReply);
