...
function ProductCard({ camera, isActive }: ProductCardProps): JSX.Element {
  ...
  const cardClass = classNames('product-card', { 'is-active': isActive });
  ...
  return (
    <div className={cardClass}>
      ...
      <div className="product-card__buttons">
        ...
        <Link className="btn btn--transparent" to={`${AppRoute.Root}${generatePath(AppRoute.Product, { id: String(camera.id) })}`}>
          Подробнее
        </Link>
      </div>
    </div>
